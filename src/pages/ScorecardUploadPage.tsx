...
export default function ScorecardUploadPage() {
  ...
  return (
    <div className="p-4 pb-10 max-w-lg mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Upload Scorecard</h1>
      ...
      {state === 'idle' ? (
        <div
          ...
          className="border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center py-14 px-6 text-center cursor-pointer hover:border-brand-300 hover:bg-brand-50 transition-colors mb-4">
          <div className="text-5xl mb-3">🖼️</div>
          <p className="font-medium text-gray-700 mb-1">Tap to choose a photo</p>
        ...
        <div className="mb-4">
          <div className="relative rounded-2xl overflow-hidden border border-gray-100 shadow-sm mb-3">
            {previewUrl && (
              <img src={previewUrl} alt="Scorecard preview" className="w-full object-contain max-h-80 bg-gray-50" />
            )}
            {state === 'uploading' && (
              <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center gap-3">
                <div className="w-10 h-10 border-4 border-brand-500 border-t-transparent rounded-full animate-spin" />
                <p className="text-sm font-medium text-gray-600">Reading scorecard…</p>
      ...
      {state === 'previewing' && (
        <div className="flex gap-3">
          <button
            onClick={handleReset}
            className="flex-1 py-3.5 rounded-xl bg-gray-100 text-gray-600 font-semibold text-sm hover:bg-gray-200 transition-colors">
            Choose different
          </button>
          <button
      ...
      {state === 'idle' && (
        <div className="flex gap-3">
          ...
          <button
            onClick={() => fileRef.current?.click()}
            className="flex-1 py-3.5 rounded-xl bg-gray-100 text-gray-700 font-semibold text-sm hover:bg-gray-200 transition-colors flex items-center justify-center gap-2">
            🖼️ Choose file
          </button>
        </div>
      )}

      <div className="mt-6 bg-gray-50 rounded-xl border border-gray-100 px-4 py-3">
