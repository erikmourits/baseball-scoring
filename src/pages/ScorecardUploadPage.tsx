  ...
  return (
    <div className="p-4 pb-10 max-w-lg mx-auto">
      <button onClick={() => navigate('/')} className="text-brand-500 dark:text-brand-100 text-sm font-medium mb-4 flex items-center gap-1">
        {t('scorecard.backGames')}
      </button>

      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-1">{t('scorecard.title')}</h1>
      ...
      {state === 'idle' ? (
        <div
          ...
          <p className="font-medium text-gray-700 dark:text-gray-300 mb-1">{t('scorecard.tapToChoose')}</p>
          <p className="text-sm text-gray-400">{t('scorecard.dragDrop')}</p>
          <p className="text-xs text-gray-300 mt-2">{t('scorecard.fileTypes')}</p>
        </div>
      ) : (
        <div className="mb-4">
          <div className="relative rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm mb-3">
            ...
            {state === 'uploading' && (
              <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center gap-3">
                ...
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{t('scorecard.reading')}</p>
                <p className="text-xs text-gray-400">{t('scorecard.readingTime')}</p>
              </div>
            )}
          </div>
      ...
      {errorMsg && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3 mb-4">
          <p className="text-sm font-medium text-red-700 dark:text-red-400">{t('scorecard.error')}</p>
      ...
      {state === 'idle' && (
        <div className="flex gap-3">
          ...
          <button
            ...
            {t('scorecard.chooseFile')}
          </button>
        </div>
      )}

      <div className="mt-6 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-700 px-4 py-3">
        <p className="text-xs font-semibold text-gray-500 mb-1">{t('scorecard.howItWorks')}</p>
        <p className="text-xs text-gray-400 leading-relaxed">
          {t('scorecard.howItWorksText')}
        </p>
      </div>
    </div>
  )
